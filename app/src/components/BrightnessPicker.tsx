...
const BrightnessPicker: React.FC<props> = ({
    color,
    onChange,
    onChangeComplete,
}) => {
    ...
    useEffect(() => {
        if (sliderRef.current)
            (
                sliderRef.current.children[0] as HTMLInputElement
            ).style.background = `
            linear-gradient(to right, #2d2d2d, 
                hsl(
                    ${(hslColor.hue / 255) * 360},
                    ${(hslColor.saturation / 255) * 100}%,
                    50%
                ), white)`;
    });
